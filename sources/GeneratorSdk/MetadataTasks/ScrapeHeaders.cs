...
--include-directory");
        ...
        {
            string autoRemapsRspPath = Path.Combine(this.GeneratedDir, "scraper.autoRemaps.generated.rsp");

            // Read existing auto remaps from previous builds to accumulate discoveries
            var mergedRemaps = new Dictionary<string, string>(StringComparer.Ordinal);
            if (File.Exists(autoRemapsRspPath))
            {
                var existingSettings = new Dictionary<string, List<string>>(StringComparer.OrdinalIgnoreCase);
                ParseRspFile(autoRemapsRspPath, existingSettings);
                if (existingSettings.TryGetValue("--remap", out var existingEntries))
                {
                    foreach (var entry in existingEntries)
                    {
                        int eq = entry.IndexOf('=');
                        if (eq > 0)
                            mergedRemaps[entry.Substring(0, eq)] = entry.Substring(eq + 1);
                    }
                }
            }

            int previousCount = mergedRemaps.Count;

            // Merge newly discovered remaps from this build
            foreach (var kvp in this.suggestedRemaps)
                mergedRemaps[kvp.Key] = kvp.Value;

            int newCount = mergedRemaps.Count - previousCount;

            if (mergedRemaps.Count == 0)
            {
                this.Log.LogMessage(MessageImportance.High, "No auto-discovered remaps to write.");
                return;
            }

            // Write the auto-generated rsp file
            Directory.CreateDirectory(Path.GetDirectoryName(autoRemapsRspPath));
            using (var writer = new StreamWriter(autoRemapsRspPath))
            {
                writer.WriteLine("--remap");
                foreach (var kv in mergedRemaps.OrderBy(kv => kv.Key, StringComparer.OrdinalIgnoreCase))
                    writer.WriteLine(kv.Key + "=" + kv.Value);
            }

            this.Log.LogMessage(MessageImportance.High,
                $"Auto-remaps: wrote {mergedRemaps.Count} entries to {Path.GetFileName(autoRemapsRspPath)} ({newCount} new this build, {previousCount} from previous builds)");
        }
