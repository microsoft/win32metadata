...
--include-directory");
        ...
        private void WriteAutoRemapsCsFile()
        {
            ...
            // Filter: only include remaps where the tag name looks like an
            // internal/compiler-generated name. This avoids incorrect remaps like
            // IUnknown→IXmlReaderInput where the tag IS the correct public name.
            var filtered = new Dictionary<string, string>(StringComparer.Ordinal);
            var skippedNotInternal = 0;
            var skippedWorse = 0;

            // Built-in defaults from PInvokeGenerator constructor — skip these
            var builtins = new HashSet<string> { "intptr_t", "ptrdiff_t", "size_t", "uintptr_t", "_GUID" };

            foreach (var kv in this.suggestedRemaps)
            {
                string tag = kv.Key;
                ...
                // Skip if typedef looks "worse" (adds _ prefix when tag doesn't have one)
                if (!tag.StartsWith("_") && typedef.StartsWith("_"))
                {
                    skippedWorse++;
                    continue;
                }
                ...
                // - Starts with _ followed by uppercase (e.g., _ACL, _SECURITY_ATTRIBUTES)
                // - Starts with tag followed by uppercase (e.g., tagRECT, tagMSG)
                // - Starts with __MIDL or __Anonymous (MIDL-generated names)
                // - Starts with ABI:: (WinRT namespace-qualified names)
                // - First char is lowercase (C-style tags like adpcmcoef_tag, sockaddr_in6)
                // - Ends with _ (trailing underscore, common C pattern like CATALOG_INFO_)
                bool looksInternal =
                    (tag.Length > 1 && tag[0] == '_' && char.IsUpper(tag[1])) ||
                    (tag.Length > 1 && tag[0] == '_' && tag[1] == '_') ||
                    (tag.StartsWith("tag") && tag.Length > 3 && char.IsUpper(tag[3])) ||
                    tag.StartsWith("__MIDL") ||
                    tag.StartsWith("__Anonymous") ||
                    tag.StartsWith("ABI::") ||
                ...
                filtered[tag] = typedef;
            }

            this.Log.LogMessage(MessageImportance.High,
                $"Auto-remaps: {this.suggestedRemaps.Count} discovered, {filtered.Count} pass filter " +
                $"(skipped: {skippedWorse} worse-name, {skippedNotInternal} not-internal-looking)");
            ...
            using (var writer = new StreamWriter(autoRemapsCsPath))
            {
                ...
                writer.WriteLine("// Filtered to only include entries where the tag name looks internal.");
            ...
            this.Log.LogMessage(MessageImportance.High,
                $"Auto-remaps: wrote {filtered.Count} entries to {Path.GetFileName(autoRemapsCsPath)}");
        }
