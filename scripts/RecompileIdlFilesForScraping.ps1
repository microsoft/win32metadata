...
$idlFiles = Get-ChildItem "$recompiledIdlHeadersDir\um\*.idl","$recompiledIdlHeadersDir\shared\*.idl"
...
$foundIdlFiles | ForEach-Object -Parallel {
    ...
    & "$using:midl" $inputFileName /out $using:scratchDir /header $outputHeader /no_warn /DUNICODE /D_UNICODE /DWINVER=0x0A00 -D_APISET_MINWIN_VERSION=0x010F /DNTDDI_VERSION=0x0A00000C /DBUILD_UMS_ENABLED=0 /DBUILD_WOW64_ENABLED=0 /DBUILD_ARM64X_ENABLED=0 /DEXECUTABLE_WRITES_SUPPORT=0 -D_USE_DECLSPECS_FOR_SAL=1 -D_CONTROL_FLOW_GUARD_SVCTAB=1 -DMIDL_PASS=1 /Di386 /D_X86_ /D_WCHAR_T_DEFINED /no_stamp /nologo  /no_settings_comment /lcid 1033 -sal /win32 -target NT100 /Zp8 /I$using:scratchDir /I$using:sdkIncludeDir\um /I$using:sdkIncludeDir\shared /I$using:sdkIncludeDir\winrt /I$using:extraIncPath 3>&1 2>&1 > $outputLog
