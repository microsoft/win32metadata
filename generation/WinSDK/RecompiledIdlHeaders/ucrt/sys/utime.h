...
_ACRTIMP int __cdecl _wutime64(
    ...
    _In_opt_ struct __utimbuf64* _Time
        ...
        _CRT_NONSTANDARD_STATIC __inline int __CRTDECL _utime(char const* const _FileName, struct _utimbuf* const _Time)
        {
            _STATIC_ASSERT(sizeof(struct _utimbuf) == sizeof(struct __utimbuf32));
            return _utime32(_FileName, (struct __utimbuf32*)_Time);
        }

        _CRT_NONSTANDARD_STATIC __inline int __CRTDECL _futime(int const _FileHandle, struct _utimbuf* const _Time)
        {
            _STATIC_ASSERT(sizeof(struct _utimbuf) == sizeof(struct __utimbuf32));
            return _futime32(_FileHandle, (struct __utimbuf32*)_Time);
        }

        _CRT_NONSTANDARD_STATIC __inline int __CRTDECL _wutime(wchar_t const* const _FileName, struct _utimbuf* const _Time)
        {
            _STATIC_ASSERT(sizeof(struct _utimbuf) == sizeof(struct __utimbuf32));
        ...
        _CRT_NONSTANDARD_STATIC __inline int __CRTDECL _wutime(wchar_t const* const _FileName, struct _utimbuf* const _Time)
        {
            ...
            _CRT_NONSTANDARD_STATIC __inline int __CRTDECL utime(char const* const _FileName, struct utimbuf* const _Time)
            {
